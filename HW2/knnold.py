...
def compute_similarity(data, metadata, k):
    length = len(metadata)
    ...
    for i in range(0, len(data)):
       ...
       # print(str(i+1)+"th record is close to:")
        ...
        text.append(data[i][class_label_index])
        for o in range(0, k):
            #print(s[o])
            text.append(s[o][0])
            text.append(s[o][1])
...
def knn(k):
    ...
    f = open('knn.csv','w+')
    writer = csv.writer(f)
    tex = ["Trans. ID","Actual Class label"," Predicted class","Posterior Probabilty"]
    writer.writerow(tex)
...
if __name__ == "__main__":
    ...
    with open(filename) as csvfile:
        ...
        compute_similarity(data, metadata, k)
